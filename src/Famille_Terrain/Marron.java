...
public class Marron extends Terrain{
    
    private static int[] famille = new int[2];

    public Marron( int id, int prixMaison, String couleur, 
    ...
    public void achetee(Plateau p, int idJoueur) {
        if(getId() == 1){
            famille[0] = idJoueur;
            p.setPropPlateau(idJoueur, 1);
        }
        else{
            famille[1] = idJoueur;
            p.setPropPlateau(idJoueur, 3);
        }
    }
    ...
    public void addMaison(Joueur j, Plateau p) {
        if(getId() == 3){
            if(getMaisonFamille()[1] > getMaisonFamille()[0]){
                p.getCasePlateau()[3].construire(j,p);
            }
            addMaison();
            ...
            if(getMaisonFamille()[0] > getMaisonFamille()[1]){
                p.getCasePlateau()[1].construire(j,p);
            }
            addMaison();
            j.payer(getPrixMaison());
        }
