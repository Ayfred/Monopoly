...
public class Vert extends Terrain{
    private static int[] famille = new int[3];

    public Vert(int id, int prixMaison, String couleur, String nom, 
    ...
    public void achetee(Plateau p,int idJoueur) {
        if(getId() == 31){
            famille[0] = idJoueur;
            p.setPropPlateau(idJoueur, 31);
        }
        else{
            if(getId() == 32){
                famille[1] = idJoueur;
                p.setPropPlateau(idJoueur, 32);
            }
            else{
                famille[2] = idJoueur;
                p.setPropPlateau(idJoueur, 33);
            }
        }
    }
    ...
    public void addMaison(Joueur j, Plateau p) {
        if(getId() == 31){
            if(getMaisonFamille()[0] > getMaisonFamille()[1]){
                p.getCasePlateau()[32].construire(j,p);
            }
            if(getMaisonFamille()[0] > getMaisonFamille()[2]){
                p.getCasePlateau()[34].construire(j,p);
            }
            addMaison();
            ...
            if(getId() == 32){
                ...
                if(getMaisonFamille()[1] > getMaisonFamille()[0]){
                    p.getCasePlateau()[31].construire(j,p);
                }
                addMaison();
                j.payer(getPrixMaison());
            }
