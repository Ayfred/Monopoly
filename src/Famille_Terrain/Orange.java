...
public class Orange extends Terrain{
    
    private static int[] famille = new int[3];

    public Orange(int id, int prixMaison, String couleur, String nom, int valeur, int[] loyers) {
    ...
    public void achetee(Plateau p, int idJoueur) {
        if(getId() == 16){
            famille[0] = idJoueur;
            p.setPropPlateau(idJoueur, 16);
        }
        else{
            if(getId() == 17){
                famille[1] = idJoueur;
                p.setPropPlateau(idJoueur, 17);
            }
            else{
                famille[2] = idJoueur;
                p.setPropPlateau(idJoueur, 19);
            }
        }
    }
    ...
    public void addMaison(Joueur j, Plateau p) {
        if(getId() == 16){
            if(getMaisonFamille()[0] > getMaisonFamille()[1]){
                p.getCasePlateau()[18].construire(j,p);
            }
            if(getMaisonFamille()[0] > getMaisonFamille()[2]){
                p.getCasePlateau()[19].construire(j,p);
            }
            addMaison();
            ...
            if(getId() == 18){
                ...
                if(getMaisonFamille()[1] > getMaisonFamille()[0]){
                    p.getCasePlateau()[16].construire(j,p);
                }
                addMaison();
                j.payer(getPrixMaison());
            }
