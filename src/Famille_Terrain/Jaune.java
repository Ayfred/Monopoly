...
public class Jaune extends Terrain {
    
    private static int[] famille = new int[3];

    public Jaune(int id, int prixMaison, String couleur, String nom, 
    ...
    public void achetee(Plateau p, int idJoueur) {
        if(getId() == 26){
            famille[0] = idJoueur;
            p.setPropPlateau(idJoueur, 26);
        }
        else{
            if(getId() == 27){
                famille[1] = idJoueur;
                p.setPropPlateau(idJoueur, 27);
            }
            else{
                famille[2] = idJoueur;
                p.setPropPlateau(idJoueur, 29);
            }
        }
    }
    ...
    public void addMaison(Joueur j, Plateau p) {
        if(getId() == 26){
            if(getMaisonFamille()[0] > getMaisonFamille()[1]){
                p.getCasePlateau()[27].construire(j,p);
            }
            if(getMaisonFamille()[0] > getMaisonFamille()[2]){
                p.getCasePlateau()[29].construire(j,p);
            }
            addMaison();
            ...
            if(getId() == 27){
                ...
                if(getMaisonFamille()[1] > getMaisonFamille()[0]){
                    p.getCasePlateau()[26].construire(j,p);
                }
                addMaison();
                j.payer(getPrixMaison());
            }
